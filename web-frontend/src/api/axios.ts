...
export const uploadImage = async (file: File): Promise<PredictionResponse> => {
  ...
  const response = await apiClient.post<PredictionResponse>('/api/predict/image', formData);
...
export const uploadVideo = async (file: File): Promise<PredictionResponse> => {
  ...
  const response = await apiClient.post<PredictionResponse>('/api/predict/video', formData);
  return response.data;
};
